        ...
        private void DoTicks(int ticks)
        {
            if (!this.Ruined)
            {
                ...
                if (this.ruinedPercent >= 1f)
                {
                    ...
                    Messages.Message(this.parent.def.label.CapitalizeFirst() + "VRE_DestroyedByTemp".Translate(), new TargetInfo(this.parent.Position, this.parent.Map, false), MessageTypeDefOf.NegativeEvent);
                    this.parent.Destroy();
